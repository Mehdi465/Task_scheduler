...
Time::Time(int d, int h, int m){
    ...
    if (d<0){
        throw std::invalid_argument("day is below 0");
    }
...
void Time::addTime(Time time_to_add){
    ...
    this->hour = this->hour + time_to_add.hour + this->minute / 60;
    this->day = this->day + time_to_add.day + this->hour/24;
    this->hour %= 24; 
    this->minute %= 60;
}
...
bool Time::isInferior(Time compared_time){
    ...
    if (this->day > compared_time.day){
        res = false;
    }
    else if (this->day == compared_time.day){
...
void Time::printTime() const{
    std::cout << this->hour << "h" << this->minute << std::endl;
}
...
Schedule::Schedule() : begin_t(Time()), end_t(Time(0,23,59)){};
...
Schedule::Schedule(const std::string file_name,Time start, Time end){
    ...
    // check if sum(probas) = 1
    if(std::abs((double)sum_proba-1.0)>0.002){
        throw std::runtime_error("Sum probas is not equal to 1");
    }
    ...
    while(current_time.isInferior(end)){
        ...
        current_time.addTime(Time(0,0,TIME_PER_TASK));
